#!/usr/bin/env python3
"""
MLBMA ODDS PIPELINE v0.9 (2026-08-02) -- STATUS: LIVE (TB alt-ladder 194/194 beat close 7/17-7/24)
v0.9: SPREADS-CLV BLIND-SPOT FIX (authorized by Grant, Session S 2026-08-02).
Closers now request alternate_spreads alongside spreads, so a runline whose
line moved grades prob-space CLV at the ORIGINAL bet point instead of n/q.
Cost: +1 credit per spreads-bet event at close. Same pattern as the v0.8
totals fix; matching logic unchanged (already point-exact for spreads).
v0.8: price math EXTRACTED to pricing.py (shared module, Session S 2026-07-23).
...
if not API_KEY:
    _kf = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".odds_key")
    API_KEY = open(_kf).read().strip() if os.path.exists(_kf) else ""
if not API_KEY:
    sys.exit("FATAL: no Odds API key. Set ODDS_API_KEY env var or decrypt the vault to .odds_key (S4).")
...
# v0.8: ALL price math lives in pricing.py -- the single canonical
# implementation every session imports (Session S 2026-07-23 ruling).
# Never define am_to_prob/prob_to_am/med_am/novig locally again.
from pricing import (
    am_to_prob, prob_to_am, novig_two_way,
    is_corrupt_am, clean_prices, med_am,
)
...
def close_markets_for(mkey):
    ...
    if mkey == "spreads":
        return "spreads,alternate_spreads"   # v0.9: moved-line runlines
    if mkey == "totals":
...
def close_for_bet(event_data, bet):
    ...
    if mkey == "batter_total_bases":
        ...
        accept.add("alternate_spreads")       # v0.9: moved-line runlines close at the bet point
    elif mkey == "totals":
...
def run_selftest():
    ...
    # v0.9: moved-line runline must grade off alternate_spreads, not come back n/q
    assert close_markets_for("spreads") == "spreads,alternate_spreads"
    ev2 = {"bookmakers": [{"markets": [
        {"key": "spreads", "outcomes": [
            {"name": "Detroit Tigers", "point": 1.5, "price": -130},
            {"name": "Athletics", "point": -1.5, "price": 110}]},
        {"key": "alternate_spreads", "outcomes": [
            {"name": "Detroit Tigers", "point": 2.5, "price": -180},
            {"name": "Athletics", "point": -2.5, "price": 155}]},
    ]}]}
    sb = {"market": "spreads", "side": "DET", "point": "2.5", "my_odds": "-150",
          "player": "", "event_id": "x", "commence_time": "2026-08-02T23:00:00Z"}
    r2 = close_for_bet(ev2, sb)
    assert r2["clv_note"] == "" and r2["close_med_side"] == -180, r2
    assert float(r2["clv_prob_pts"]) > 0, r2  # DET +2.5 at -150 vs -180 close = beat the close
